...
public class AcaciaCrateTileEntity extends LockableLootTileEntity implements INamedContainerProvider
{
    ...
    public AcaciaCrateTileEntity() {
        super((TileEntityType)ModTileEntities.ACACIA_STORAGE_CRATE_TE);
    ...
    protected ITextComponent getDefaultName() {
        return (ITextComponent)new TranslationTextComponent("Acacia Crate", new Object[0]);
    }
    ...
    public static int getPlayersUsing(final IBlockReader reader, final BlockPos posIn) {
        ...
        if (blockstate.hasTileEntity()) {
            ...
            if (tileentity instanceof AcaciaCrateTileEntity) {
                return ((AcaciaCrateTileEntity)tileentity).numPlayersUsing;
            }
        }
        return 0;
    }
    ...
    public void onScheduledTick() {
        ...
        if (world != null) {
            ...
            if (this.playerCount > 0) {
                ...
                if (!(blockState.getBlock() instanceof BlockAcaciaCrate)) {
                    this.remove();
                }
            }
        }
    ...
    private void updatePlayerCount(final World world, final IInventory inventory, final int x, final int y, final int z) {
        this.playerCount = 0;
        for (final PlayerEntity playerEntity : world.getEntitiesWithinAABB(PlayerEntity.class, new AxisAlignedBB((double)(x - 5.0f), (double)(y - 5.0f), (double)(z - 5.0f), (double)(x + 1 + 5.0f), (double)(y + 1 + 5.0f), (double)(z + 1 + 5.0f)))) {
            if (playerEntity.openContainer instanceof ChestContainer) {
                final IInventory crateInventory = (IInventory)playerEntity.openContainer;
