...
public abstract class AbstractObsidianFurnaceBlock extends AbstractFurnaceBlock
{
    public static final DirectionProperty FACING;
    public static final BooleanProperty LIT;
    
    protected AbstractObsidianFurnaceBlock(final Block.Properties p_i50000_1_) {
        super(p_i50000_1_);
        this.setDefaultState((BlockState)((BlockState)((BlockState)this.stateContainer.getBaseState()).with((IProperty)AbstractObsidianFurnaceBlock.FACING, (Comparable)Direction.NORTH)).with((IProperty)AbstractObsidianFurnaceBlock.LIT, (Comparable)false));
    }
    
    protected void interactWith(final World worldIn, final BlockPos pos, final PlayerEntity player) {
        final TileEntity tileentity = worldIn.getTileEntity(pos);
        if (tileentity instanceof AbstractObsidianFurnaceTileEntity) {
    ...
    public void animateTick(final BlockState stateIn, final World worldIn, final BlockPos pos, final Random rand) {
        if (stateIn.get(AbstractObsidianFurnaceBlock.LIT)) {
            ...
            final Direction direction = (Direction)stateIn.get((IProperty)AbstractObsidianFurnaceBlock.FACING);
    ...
    public void onBlockPlacedBy(final World worldIn, final BlockPos pos, final BlockState state, final LivingEntity placer, final ItemStack stack) {
        if (stack.hasDisplayName()) {
            final TileEntity tileentity = worldIn.getTileEntity(pos);
            if (tileentity instanceof AbstractObsidianFurnaceTileEntity) {
                ((AbstractObsidianFurnaceTileEntity)tileentity).setCustomName(stack.getDisplayName());
            }
        }
    }
