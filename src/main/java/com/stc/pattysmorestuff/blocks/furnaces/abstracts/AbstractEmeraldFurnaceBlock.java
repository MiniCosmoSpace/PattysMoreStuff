...
public abstract class AbstractEmeraldFurnaceBlock extends AbstractFurnaceBlock
{
    public static final DirectionProperty FACING;
    public static final BooleanProperty LIT;
    
    protected AbstractEmeraldFurnaceBlock(final Block.Properties p_i50000_1_) {
        super(p_i50000_1_);
        this.setDefaultState((BlockState)((BlockState)((BlockState)this.stateContainer.getBaseState()).with((IProperty)AbstractEmeraldFurnaceBlock.FACING, (Comparable)Direction.NORTH)).with((IProperty)AbstractEmeraldFurnaceBlock.LIT, (Comparable)false));
    }
    
    protected void interactWith(final World worldIn, final BlockPos pos, final PlayerEntity player) {
        final TileEntity tileentity = worldIn.getTileEntity(pos);
        if (tileentity instanceof AbstractEmeraldFurnaceTileEntity) {
    ...
    public void animateTick(final BlockState stateIn, final World worldIn, final BlockPos pos, final Random rand) {
        if (stateIn.get(AbstractEmeraldFurnaceBlock.LIT)) {
            ...
            final Direction direction = (Direction)stateIn.get((IProperty)AbstractEmeraldFurnaceBlock.FACING);
    ...
    public void onBlockPlacedBy(final World worldIn, final BlockPos pos, final BlockState state, final LivingEntity placer, final ItemStack stack) {
        if (stack.hasDisplayName()) {
            final TileEntity tileentity = worldIn.getTileEntity(pos);
            if (tileentity instanceof AbstractEmeraldFurnaceTileEntity) {
                ((AbstractEmeraldFurnaceTileEntity)tileentity).setCustomName(stack.getDisplayName());
            }
        }
    }
