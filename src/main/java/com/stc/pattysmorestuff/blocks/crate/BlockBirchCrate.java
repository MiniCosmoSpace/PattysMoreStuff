...
public class BlockBirchCrate extends ContainerBlock
{
    public BlockBirchCrate(final String name, final Block.Properties p_i48430_1_) {
        super(p_i48430_1_);
    ...
    public TileEntity createNewTileEntity(final IBlockReader worldIn) {
        return (TileEntity)new BirchCrateTileEntity();
    }
    ...
    public void onReplaced(final BlockState state, final World worldIn, final BlockPos pos, final BlockState newState, final boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (tileentity instanceof BirchCrateTileEntity) {
                ((BirchCrateTileEntity)tileentity).removeAdornments();
