...
public class BlockOakCrate extends ContainerBlock
{
    public BlockOakCrate(final String name, final Block.Properties properties) {
        super(properties);
    ...
    public TileEntity createNewTileEntity(final IBlockReader worldIn) {
        return (TileEntity)new OakCrateTileEntity();
    }
    ...
    public void onReplaced(final BlockState state, final World worldIn, final BlockPos pos, final BlockState newState, final boolean isMoving) {
        if (state.getBlock() != newState.getBlock()) {
            ...
            if (tileentity instanceof OakCrateTileEntity) {
                ((OakCrateTileEntity)tileentity).removeAdornments();
