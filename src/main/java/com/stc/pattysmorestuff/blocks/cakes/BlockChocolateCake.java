...
public class BlockChocolateCake extends Block
{
    ...
    public BlockChocolateCake(final String name, final Block.Properties properties) {
        super(properties);
        this.setRegistryName("pattysmorestuff", name);
        this.setDefaultState((BlockState)((BlockState)this.stateContainer.getBaseState()).with((IProperty)BlockChocolateCake.BITES, (Comparable)0));
    ...
    public VoxelShape getShape(final BlockState state, final IBlockReader worldIn, final BlockPos pos, final ISelectionContext context) {
        return BlockChocolateCake.SHAPES[(int)state.get((IProperty)BlockChocolateCake.BITES)];
    }
    ...
    private boolean eatCake(final IWorld worldIn, final BlockPos pos, final BlockState state, final PlayerEntity player) {
        ...
        final int i = (int)state.get((IProperty)BlockChocolateCake.BITES);
        if (i < 6) {
            worldIn.setBlockState(pos, (BlockState)state.with((IProperty)BlockChocolateCake.BITES, (Comparable)(i + 1)), 3);
        }
        else {
    ...
    protected void fillStateContainer(final StateContainer.Builder<Block, BlockState> builder) {
        builder.add(new IProperty[] { (IProperty)BlockChocolateCake.BITES });
    }
    
    public int getComparatorInputOverride(final BlockState blockState, final World worldIn, final BlockPos pos) {
        return (7 - (int)blockState.get((IProperty)BlockChocolateCake.BITES)) * 2;
    }
